...
## Constructing a knowledge scale with `info_scale()`
...
## Evaluating construct validity with `info_emmeans()`
...
## Calculating propensity scores with `info_prop_scores()`
...
## Evaluating propensity scores using `info_bal_plots()`
...
## Calculating information effects with `info_effect()`
...
We started out by noting that information matters for politics, and that one of the main ways of determining the difference that knowledge will make in any given instance is by modeling information effects. The `informationeffects`  offers a complete pipeline for calculating such effects. If anyone spots any bugs or issues, please log them on the GitHub repo [here](https://github.com/ahlstromvij/informationeffects/issues).
